...
TileMap::TileMap(std::string tileSheetFileName, int rows, int cols, int _TileWidth, int _TileHeight, int _mapX, int _mapY, int* data, SDL_Renderer* ren){
    if(nullptr==ren){
    ...
    for(int i=0; i < m_MapX*m_MapY; i++){
        m_Tiles[i] = data[i] - 1; //! Default value is no tile.
    }
}
...
//! render TileMap
void TileMap::Render(SDL_Renderer* ren, int x_scroll, int y_scroll){
    if(nullptr==ren){
        SDL_Log("No valid renderer found");
    }
    ...
    for(int y= 0; y < m_MapY; y++){
        for(int x= 0; x < m_MapX; x++){
            ...
            if(currentTile > -1 ){
                ...
                Dest.x = x*m_TileWidth - x_scroll;
                Dest.y = y*m_TileHeight - y_scroll;
                Dest.w = m_TileWidth;
                Dest.h = m_TileHeight;
